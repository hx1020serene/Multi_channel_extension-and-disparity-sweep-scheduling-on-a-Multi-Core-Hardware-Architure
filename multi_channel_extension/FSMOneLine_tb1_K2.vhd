...
entity FSMOneLine_tb1_K2 is
end FSMOneLine_tb1_K2;

architecture Behavioral of FSMOneLine_tb1_K2 is
    ...
    -- K=2 configuration
    signal last_layer_output_channels : integer := 2;
...
begin
    ...
    stimulus: process begin
        ...
        --------------------------------------------------------------------------------
        -- (2) Write Weights Memory 2 (Mode 0100) - K=2 Configuration
        --------------------------------------------------------------------------------
        mode <= "0100"; 
        nb_of_addresses_to_write <= "0000000100"; --  4 addresses (0-3) for K=2
        
        for i in 0 to 3 loop
            if i = 0 then start <= '1'; else start <= '0'; end if;
            
            if (i = 0) or (i = 1) then        
                -- Address 0, 1: Ch0 will use Address 0
                input_data <= x"3C003C003C003C00"; -- 1.0
            elsif (i = 2) or (i = 3) then    
                -- Address 2, 3: Ch1 will use Address 2
        ...
        --------------------------------------------------------------------------------
        -- (3) Write Bias Memory 2 (Mode 0110) - SIMD Bias
        --------------------------------------------------------------------------------
        mode <= "0110"; 
        nb_of_addresses_to_write <= "0000000011"; -- 3 addresses 

        for i in 0 to 2 loop
            if i = 0 then start <= '1'; else start <= '0'; end if;
            
            if i = 0 then
                -- Ch0: Target output = 5.0 (4.0 + 1.0)
                input_data <= x"3C003C003C003C00"; -- 1.0
            elsif i = 1 then
                -- Ch1: Target output = 10.0 (4.0¡Á2.0 + 2.0)
                input_data <= x"4000400040004000"; -- 2.0
            else
                -- Ch2: Unused for K=2
        ...
        --------------------------------------------------------------------------------
        -- (3.5) Write Weights Memory 1 (Mode 0011) - Main Core Bypass
        --------------------------------------------------------------------------------
        mode <= "0011"; 
        ...
        for i in 0 to 11 loop
            if i = 0 then start <= '1'; else start <= '0'; end if;
            input_data_weights <= x"3C003C003C003C003C003C003C003C003C003C003C003C003C003C003C003C00"; -- 1.0
            wait for 10 ns;
        end loop;
        ...
        --------------------------------------------------------------------------------
        -- (3.6) Write Bias Memory 1 (Mode 0101) - Main Core Bypass
        --------------------------------------------------------------------------------
        mode <= "0101"; 
        ...
        --------------------------------------------------------------------------------
        -- (4) EXECUTE Mode 1011 (SIMD Execution) - K=2
        --------------------------------------------------------------------------------
        mode <= "1011";
        must_be_written_back <= "00";
